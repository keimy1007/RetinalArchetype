...
def _torch_nnls(A, b):
    # 簡易的なNNLS実装。高速化や最適化は行っていないので、必要に応じて改善が必要です。
    x, _ = torch.lstsq(b, A)
    x = x[:A.size(1)]
    x[x < 0] = 0
    return x
...
def _optimize_alphas(B, A):
    device = B.device  # GPUデバイスを取得
    B = torch.nn.functional.pad(B, (0, 1), value=200)
    A = torch.nn.functional.pad(A, (0, 1), value=200)
    alphas = torch.empty((B.shape[0], A.shape[0]), device=device)
    for j in range(alphas.T.shape[1]):
        alphas.T[:, j] = _torch_nnls(A.T, B.T[:, j])
    alphas /= alphas.sum(1).unsqueeze(1)
    alphas[torch.isnan(alphas)] = 1 / alphas.shape[1]
    return alphas
...
def _aa_simple(X, i_alphas, i_betas, max_iter, tol, verbose=False):
    device = X.device
    alphas = torch.tensor(i_alphas, device=device)
    betas = torch.tensor(i_betas, device=device)
    ...
    for n_iter in range(max_iter):
        ...
        Z = torch.pinverse(alphas) @ X
        betas = _optimize_betas(Z, X)
        Z = betas @ X
        rss = torch.linalg.norm(X - alphas @ Z, 'fro')  # Frobenius norm
        if abs(rss_0 - rss) < tol:
            break
        rss_0 = rss

    return alphas.cpu().numpy(), betas.cpu().numpy(), rss_0, Z.cpu().numpy(), n_iter  # 最後にnumpy配列に変換して戻す



class AA_GPU(BaseEstimator, TransformerMixin):
    """
    ...
    def _check_parameters(self):
     ...
     def _init_coefs(self, X, random_state):
        X = torch.tensor(X, device="cuda")  # GPUにデータを移動

        ind = random_state.choice(self.n_archetypes, X.shape[0])
        alphas = torch.zeros(X.shape[0], self.n_archetypes, dtype=torch.float64, device="cuda")
        for i, j in enumerate(ind):
            alphas[i, j] = 1

        betas = torch.zeros(self.n_archetypes, X.shape[0], dtype=torch.float64, device="cuda")
        if self._algorithm_init == "random":
            ind = random_state.choice(X.shape[0], self.n_archetypes)
        else:
            ind = furthest_sum(X.T.cpu().numpy(), self.n_archetypes, random_state)  # PyTorchテンソルからnumpy配列に変換
        for i, j in enumerate(ind):
            betas[i, j] = 1

        return alphas.cpu().numpy(), betas.cpu().numpy()  # numpy配列に変換して戻す
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        self : object
            Fitted estimator.
        """
        X = torch.tensor(self._validate_data(X, dtype=[np.float64, np.float32]), device="mps")  # GPUにデータを移動
